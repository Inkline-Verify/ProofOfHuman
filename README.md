...
Attest API reports unsupported for native Mac apps today (expected in macOS
27); the notary-side verification for it was built and tested in this
project's development history and will be restored when Apple flips the
switch. A receipt does not prove who composed the words, only that a person
approved them.
